    ...
    public partial class Login : System.Web.UI.Page
    {
        ...
        protected void EditBtn_Click(object sender, EventArgs e)
        {
            ...
            else if (RoleCb.SelectedIndex == 1)
            {
                ...
                {
                    ErrMsg.InnerText = "Invalide Admin !!!";
                }
            }
            else if (RoleCb.SelectedIndex == 2)
            {
                ...
                if (dt.Rows.Count == 0)
                {
                    ErrMsg.InnerText = "Invalide Doctor !!!";
                }
                else
                {
                    string role = "Doctors";
                    ...
                    Session["role"] = role;
                    Session.Timeout = 10;
                    string r_url = "{0}/Precreptions.aspx";
                    r_url = string.Format(r_url, role);
            ...
            else if (RoleCb.SelectedIndex == 3)
            {
                ...
                if (dt.Rows.Count == 0)
                {
                    ErrMsg.InnerText = "Invalide Receptionist !!!";
                }
                else
                {
            ...
            else if (RoleCb.SelectedIndex == 4)
            {
                ...
                string Query = "Select * from  LaboratirianTbl where LabEmail = '{0}' and LabPass = '{1}' ";
                Query = string.Format(Query, EmailTb.Value, PasswordTb.Value);
                DataTable dt = Con.GetDatas(Query);
                if (dt.Rows.Count == 0)
                {
                    ErrMsg.InnerText = "Invalide Labroterians !!!";
                }
                else
                {
                    string role = "Labroterians";
                    Session["uid"] = dt.Rows[0][0].ToString();
                    Session["role"] = role;
                    Session.Timeout = 10;
